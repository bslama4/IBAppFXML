...
import ib_app.IB_AppFxmlController;
//! [ewrapperimpl]
public class EWrapperImpl implements EWrapper {
	...
	protected int currentOrderId = -1;
        ...
        ib_app.SimpleMovingAverage mavgShort = new ib_app.SimpleMovingAverage(50);
        ib_app.SimpleMovingAverage mavgLong = new ib_app.SimpleMovingAverage(200);
    
	//! [socket_declare]
	
	//! [socket_init]
	public EWrapperImpl(TextArea area) {
		readerSignal = new EJavaSignal();
		clientSocket = new EClientSocket(this, readerSignal);
                textArea = area;
                System.out.println("periods are - " + mavgShort.getPeriod() + " " + mavgLong.getPeriod());
                //textArea.appendText("periods are - " + mavgShort.getPeriod() + " " + mavgLong.getPeriod() + "\n");	
        }
        ...
        public EWrapperImpl(IB_AppFxmlController controller) {
		readerSignal = new EJavaSignal();
		clientSocket = new EClientSocket(this, readerSignal);
                //textArea = controller.;
	}
	...
	public void nextValidId(int orderId) {
		System.out.println("Next Valid Id: ["+orderId+"]");
                textArea.appendText("Next Valid Id is : ["+orderId+"]\n\n");
                
		//.appendText("Next Valid Id: ["+orderId+"]");
	...
	public void historicalData(int reqId, Bar bar) {
		System.out.println("HistoricalData. "+reqId+" - Date: "+bar.time()+", Open: "+bar.open()+", High: "+bar.high()+", Low: "+bar.low()+", Close: "+bar.close()+", Volume: "+bar.volume()+", Count: "+bar.count()+", WAP: "+bar.wap());
                textArea.appendText("\nHistoricalData. "+reqId+" - Date: "+bar.time()+", Open: "+bar.open()+", High: "+bar.high()+", Low: "+bar.low()+", Close: "+bar.close()+", Volume: "+bar.volume()+", Count: "+bar.count()+", WAP: "+bar.wap());
                mavgShort.addData(bar.high());
                mavgLong.addData(bar.high());
                if(this.noOfPoints > mavgLong.getPeriod())
                {
                    System.out.println("MA Short and Long are - " + mavgShort.getMean() + " , " + mavgLong.getMean());
                    if(mavgShort.getMean() > mavgLong.getMean())
                    {
                        System.out.println("BuyZone");
                    }
                    else
                    {
                        System.out.println("SellZone");
                    }
                }
        }
	//! [historicaldata]
	...
	public void error(int id, int errorCode, String errorMsg) {
		System.out.println("Error. Id: " + id + ", Code: " + errorCode + ", Msg: " + errorMsg + "\n");
                //textArea.appendText("Error. Id: " + id + ", Code: " + errorCode + ", Msg: " + errorMsg + "\n");
        }
	//! [error]
	@Override
